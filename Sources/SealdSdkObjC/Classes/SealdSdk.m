                                                               ...
                                                               error:(NSError*_Nullable*)error
{
    ...
    SealdSdkInternalsMobile_sdkGroupTMRTemporaryKey* res = [sdkInstance createGroupTMRTemporaryKey:(NSString*)groupId
                                                                                        authFactor:[authFactor toMobileSdk]
                                                                                           isAdmin:isAdmin
                                                                              rawOverEncryptionKey:(NSData*)overEncryptionKey
                                                                                             error:&localErr];
    if (localErr) {
        _SealdInternal_ConvertError(localErr, error);
        return nil;
...
{
    NSError* localErr = nil;
    [sdkInstance convertGroupTMRTemporaryKey:(NSString*)groupId temporaryKeyId:(NSString*)temporaryKeyId tmrJWT:(NSString*)tmrJWT rawOverEncryptionKey:(NSData*)overEncryptionKey deleteOnConvert:deleteOnConvert error:&localErr];
    if (localErr) {
        _SealdInternal_ConvertError(localErr, error);
    }
}
                                   ...
                                   completionHandler:(void (^)(NSError*_Nullable error))completionHandler
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSError* localErr = nil;
        [self convertGroupTMRTemporaryKeyWithGroupId:groupId temporaryKeyId:temporaryKeyId tmrJWT:tmrJWT overEncryptionKey:overEncryptionKey deleteOnConvert:deleteOnConvert error:&localErr];
        completionHandler(localErr);
    });
}
