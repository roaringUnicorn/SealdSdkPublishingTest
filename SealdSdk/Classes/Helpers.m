...
- (instancetype) initWithNbPage:(long)nbPage
...
+ (SealdListedGroupTMRTemporaryKeys*) fromMobileSdk:(SealdSdkInternalsMobile_sdkListedGroupTMRTemporaryKeys*)nativeList
{
    NSMutableArray<SealdGroupTmrTemporaryKey*>* localArray = [NSMutableArray arrayWithCapacity:[nativeList.keys size]];
    for (int i = 0; i < [nativeList.keys size]; i++) {
        [localArray addObject:[SealdGroupTmrTemporaryKey fromMobileSdk:[nativeList.keys get:i]]];
    }
    return [[SealdListedGroupTMRTemporaryKeys alloc] initWithNbPage:nativeList.nbPage keys:localArray];
}
@end
